...
public sealed class SpeedtestRunServiceTests : IDisposable
{
    ...
    public async Task ManualRuns_NeedFullAccess()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var runner = new FailsFirstRunner();
        _services = await RunTestServices.BuildAsync(_connection, runner, cancellationToken, access: FixedAccess.ReadOnly);
    ...
    {
        _services = await RunTestServices.BuildAsync(_connection, runner, cancellationToken);
        return NewRuns();
    }
