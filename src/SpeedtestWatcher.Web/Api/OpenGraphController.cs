...
{
    public const string ImagePath = "api/opengraph/image";
    ...
    private const float LabelBaseline = 110;
    private const float ValueBaseline = 212;
    private const float CaptionBaseline = 268;
    private const float ValueFontSize = 80;
    ...
    public async Task<IActionResult> GetImage(CancellationToken cancellationToken)
    {
        ...
        using var labelFont = new SKFont(SKTypeface.FromFamilyName("sans-serif", SKFontStyle.Normal), 24);
        using var valueFont = new SKFont(SKTypeface.FromFamilyName("sans-serif", SKFontStyle.Bold), ValueFontSize);
    ...
    private static void DrawCard(SKCanvas canvas, float left, string title, string value, SKPaint valuePaint, string? caption, CardStyle style)
    {
        ...
        canvas.DrawText(title, textLeft, CardTop + LabelBaseline, SKTextAlign.Left, style.LabelFont, style.LabelPaint);

        style.ValueFont.Size = ValueFontSize;
        var room = CardWidth - (2 * CardPadding);
        var width = style.ValueFont.MeasureText(value);
        if (width > room) style.ValueFont.Size = ValueFontSize * room / width;
        canvas.DrawText(value, textLeft, CardTop + ValueBaseline, SKTextAlign.Left, style.ValueFont, valuePaint);

        if (caption != null)
        {
            canvas.DrawText(caption, textLeft, CardTop + CaptionBaseline, SKTextAlign.Left, style.LabelFont, style.LabelPaint);
        }
    }
}
