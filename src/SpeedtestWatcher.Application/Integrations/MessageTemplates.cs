...
internal sealed record MessageTemplates(string Finished, string Failed, string Unhealthy, string HealthyAgain, string Skipped, string ConnectionLost, string ConnectionRestored)
{
    public static MessageTemplates DiscordMarkdown { get; } = new(
        ...
        ":red_circle: **The internet connection is down**\n > `Since`: %since%",
        ":green_circle: **The internet connection is back**\n > `Down for`: %downtime%");
    ...
    public static MessageTemplates PlainText { get; } = new(
        ...
        "The internet connection is down. Since: %since%",
        "The internet connection is back after %downtime%.");
    ...
    public string For(MessageKind kind) => kind switch
    {
        ...
        MessageKind.ConnectionLost => ConnectionLost,
        MessageKind.ConnectionRestored => ConnectionRestored,
        _ => Skipped
    };
}
