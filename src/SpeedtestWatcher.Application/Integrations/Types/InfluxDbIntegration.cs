...
internal sealed class InfluxDbIntegration : HttpIntegration
{
    ...
    private static string Line(Speedtest test, IntegrationSettings settings)
    {
        ...
        if (test.BufferbloatDown is { } bufferbloatDown) line.Append(CultureInfo.InvariantCulture, $",bufferbloat_down={bufferbloatDown:F2}");
        if (test.BufferbloatUp is { } bufferbloatUp) line.Append(CultureInfo.InvariantCulture, $",bufferbloat_up={bufferbloatUp:F2}");
